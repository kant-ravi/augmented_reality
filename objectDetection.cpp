...
int main()
{
	...
	float badCount = 0.0,correctedCount=0.0;
	Mat refImg1 = imread("reference.jpg");
	Mat refImg;
	cvtColor(refImg1,refImg,CV_BGR2GRAY);


	Mat grass = imread("grass.jpg");
	Mat wall  = imread("wall.jpg");

	//resize(sky1,sky,Size(),0.13,0.16,CV_INTER_AREA);
	//resize(grass1,grass,Size(),0.83,0.63,CV_INTER_AREA);
	//imshow("s",sky);
	//	waitKey(0);
	//	imshow("g",grass);
	//		waitKey(0);
	...
	housePoints[0] = Point3f(-1.5,1.0,0.0);
	housePoints[1] = Point3f(1.5,1.0,0.0);
	housePoints[2] = Point3f(1.5,-1.0,0.0);
	housePoints[3] =  Point3f(-1.5,-1.0,0.0);

	housePoints[4] = Point3f(-1.5,1.0,3.0);
	housePoints[5] = Point3f(1.5,1.0,3.0);
	housePoints[6] = Point3f(1.5,-1.0,3.0);
	housePoints[7] =  Point3f(-1.5,-1.0,3.0);

	housePoints[8] =  Point3f(-1.5,0.0,4.0);
	housePoints[9] =  Point3f(1.5,0.0,4.0);
	...
	for(int h = 0; h < nFrames; h++)
	{
		...
		if(matched_objectKeyPoint.size() > 20){
			...
			if((dotProd1>-0.7 && dotProd1<0.7) && (dotProd2>-0.7 && dotProd2<0.7) && (dotProd3>-0.7 && dotProd3<0.7) && (dotProd4>-0.7 && dotProd4<0.7) ) {// this program fails if 1st frame does not have the object
				...
				pts[2] = Point(housePointsRP.at<float>(6,0),housePointsRP.at<float>(6,1));
				pts[3] = Point(housePointsRP.at<float>(7,0),housePointsRP.at<float>(7,1));
				fillConvexPoly(resultImg,pts,Scalar(193,249,253),8,0);

				// side wall left
				pts[0] = Point(housePointsRP.at<float>(0,0),housePointsRP.at<float>(0,1));
				pts[1] = Point(housePointsRP.at<float>(3,0),housePointsRP.at<float>(3,1));
				pts[2] = Point(housePointsRP.at<float>(7,0),housePointsRP.at<float>(7,1));
				...
				pts[1] = Point(housePointsRP.at<float>(2,0),housePointsRP.at<float>(2,1));
				pts[2] = Point(housePointsRP.at<float>(6,0),housePointsRP.at<float>(6,1));
				pts[3] = Point(housePointsRP.at<float>(5,0),housePointsRP.at<float>(5,1));
				fillConvexPoly(resultImg,pts,Scalar(176,231,235),8,0);
				//front wall
				pts[0] = Point(housePointsRP.at<float>(0,0),housePointsRP.at<float>(0,1));
				...
				//roof
				pts[0] = Point(housePointsRP.at<float>(4,0),housePointsRP.at<float>(4,1));
				pts[1] = Point(housePointsRP.at<float>(5,0),housePointsRP.at<float>(5,1));
				pts[2] = Point(housePointsRP.at<float>(9,0),housePointsRP.at<float>(9,1));
				pts[3] = Point(housePointsRP.at<float>(8,0),housePointsRP.at<float>(8,1));

				fillConvexPoly(resultImg,pts,Scalar(80,119,154),8,0);
				//roof
				pts[0] = Point(housePointsRP.at<float>(7,0),housePointsRP.at<float>(7,1));
				pts[1] = Point(housePointsRP.at<float>(6,0),housePointsRP.at<float>(6,1));
				pts[2] = Point(housePointsRP.at<float>(9,0),housePointsRP.at<float>(9,1));
				pts[3] = Point(housePointsRP.at<float>(8,0),housePointsRP.at<float>(8,1));

				fillConvexPoly(resultImg,pts,Scalar(80,119,154),8,0);
				// wire frame house
				line( resultImg, Point(housePointsRP.at<float>(0,0),housePointsRP.at<float>(0,1)), Point(housePointsRP.at<float>(1,0),housePointsRP.at<float>(1,1)) , Scalar( 0, 0, 0), 1 );
				line( resultImg, Point(housePointsRP.at<float>(1,0),housePointsRP.at<float>(1,1)), Point(housePointsRP.at<float>(2,0),housePointsRP.at<float>(2,1)), Scalar( 0, 0, 0), 1 );
				line( resultImg, Point(housePointsRP.at<float>(2,0),housePointsRP.at<float>(2,1)), Point(housePointsRP.at<float>(3,0),housePointsRP.at<float>(3,1)) , Scalar( 0, 0, 0), 1 );
				line( resultImg, Point(housePointsRP.at<float>(3,0),housePointsRP.at<float>(3,1)), Point(housePointsRP.at<float>(0,0),housePointsRP.at<float>(0,1)) , Scalar( 0, 0, 0), 1 );

				line( resultImg, Point(housePointsRP.at<float>(4,0),housePointsRP.at<float>(4,1)), Point2f(housePointsRP.at<float>(0,0),housePointsRP.at<float>(0,1)) , Scalar( 0, 0, 0), 1 );
				line( resultImg, Point(housePointsRP.at<float>(5,0),housePointsRP.at<float>(5,1)), Point2f(housePointsRP.at<float>(1,0),housePointsRP.at<float>(1,1)) , Scalar( 0, 0, 0), 1 );
				line( resultImg, Point(housePointsRP.at<float>(6,0),housePointsRP.at<float>(6,1)), Point2f(housePointsRP.at<float>(2,0),housePointsRP.at<float>(2,1)) , Scalar( 0, 0, 0), 1 );
				line( resultImg, Point(housePointsRP.at<float>(7,0),housePointsRP.at<float>(7,1)), Point2f(housePointsRP.at<float>(3,0),housePointsRP.at<float>(3,1)) , Scalar( 0, 0, 0), 1 );
				...
				line( resultImg, Point2f(housePointsRP.at<float>(5,0),housePointsRP.at<float>(5,1)), Point2f(housePointsRP.at<float>(6,0),housePointsRP.at<float>(6,1)) , Scalar( 0, 0, 0), 1 );
				line( resultImg, Point2f(housePointsRP.at<float>(6,0),housePointsRP.at<float>(6,1)), Point2f(housePointsRP.at<float>(7,0),housePointsRP.at<float>(7,1)) , Scalar( 0, 0, 0), 1 );
				line( resultImg, Point2f(housePointsRP.at<float>(7,0),housePointsRP.at<float>(7,1)), Point2f(housePointsRP.at<float>(4,0),housePointsRP.at<float>(4,1)) , Scalar( 0, 0, 0), 1 );


				line( resultImg, Point2f(housePointsRP.at<float>(4,0),housePointsRP.at<float>(4,1)), Point2f(housePointsRP.at<float>(8,0),housePointsRP.at<float>(8,1)) , Scalar( 0, 0, 0), 1 );
				line( resultImg, Point2f(housePointsRP.at<float>(7,0),housePointsRP.at<float>(7,1)), Point2f(housePointsRP.at<float>(8,0),housePointsRP.at<float>(8,1)) , Scalar( 0, 0, 0), 1 );
				line( resultImg, Point2f(housePointsRP.at<float>(5,0),housePointsRP.at<float>(1,1)), Point2f(housePointsRP.at<float>(9,0),housePointsRP.at<float>(9,1)) , Scalar( 0, 0, 0), 1 );
				line( resultImg, Point2f(housePointsRP.at<float>(6,0),housePointsRP.at<float>(2,1)), Point2f(housePointsRP.at<float>(9,0),housePointsRP.at<float>(9,1)) , Scalar( 0, 0, 0), 1 );
				line( resultImg, Point2f(housePointsRP.at<float>(8,0),housePointsRP.at<float>(4,1)), Point2f(housePointsRP.at<float>(9,0),housePointsRP.at<float>(9,1)) , Scalar( 0, 0, 0), 1 );
		...
		else
		{
			badCount++;
			//outcap<<sceneImg1;
			cout<<"MAKING A SECOND ATTEMPT for "<<nFrames<<" frames\n";
			Mat tempRefImage, tempRefKeyPointDescriptor;
			vector<KeyPoint> tempRefKeyPoints;

			warpPerspective(refImg, tempRefImage, homography, sceneImg.size(), INTER_LINEAR, BORDER_CONSTANT);
			imshow("d",tempRefImage);
			waitKey(0);
			detector.detect(tempRefImage,tempRefKeyPoints);

			// extract descriptors of keypoint for matching with descriptors of refrence keypoints
			extractor.compute(tempRefImage, tempRefKeyPoints, tempRefKeyPointDescriptor);

			// obtaining matches b/w current frame and reference image
			vector<DMatch> tempMatchingPairs;
			matcher.match(tempRefKeyPointDescriptor, sceneKeyPointDescriptor, tempMatchingPairs);
			vector<vector<DMatch> > matches;
			vector<DMatch> goodMatches;

			matcher.knnMatch(tempRefKeyPointDescriptor, sceneKeyPointDescriptor,matches,2);
			vector<Point2f> matched_objectKeyPoint, matched_sceneKeyPoint;
			for(int i = 0; i < matches.size(); i++) {
				if(matches[i][0].distance < 0.6 * matches[i][1].distance){
					goodMatches.push_back(matches[i][0]);
					matched_objectKeyPoint.push_back(tempRefKeyPoints[matches[i][0].queryIdx].pt);
					matched_sceneKeyPoint.push_back(sceneKeyPoints[matches[i][0].trainIdx].pt);
				}
			}
			Mat matchedImage;
			drawMatches(tempRefImage,tempRefKeyPoints,sceneImg,sceneKeyPoints,goodMatches,matchedImage,Scalar::all(-1), Scalar::all(-1),
					vector<char>(), DrawMatchesFlags::NOT_DRAW_SINGLE_POINTS );
			namedWindow("Result1", WINDOW_NORMAL);
			imshow("Result1",matchedImage);
			waitKey(0);
			if(matched_objectKeyPoint.size() > 4){

				Mat transHomography = findHomography( matched_objectKeyPoint, matched_sceneKeyPoint, CV_RANSAC);

				homography = homography * transHomography;
				...
				if((dotProd1>-0.7 && dotProd1<0.7) && (dotProd2>-0.7 && dotProd2<0.7) && (dotProd3>-0.7 && dotProd3<0.7) && (dotProd4>-0.7 && dotProd4<0.7) ) {// this program fails if 1st frame does not have the object
					...
					outcap<<resultImg1;
					cout<<" \tSECOND ATTEMPT SUCCESSFUL for "<<nFrames<<" frames\n";
					correctedCount++;
				}
				else
				{
					outcap<<sceneImg1;
					cout<<" FAILED EVEN second Time poor homography"<<nFrames<<" frames\n";
				}
				...
				cout<<"FAILED SECOND ATTEMPT due to no matches";
			}
		}
	...
	cout << correctedCount<<"/"<<badCount << endl;
}// end of main
